...
选型先看两件事：**主干是随改动逐步接收代码，还是等发布批次稳定后再接收；主干之外，主要验证单项改动，还是显式组织多功能组合。**

![五种流程的典型定位：本篇先组织共享组合测试，再让已放行改动分别回主干；AoneFlow 和经典 GitFlow 更强调发布批次回主干](/assets/img/sprint-git-flow/workflow-map.webp)

图中五个点是典型用法的定性示意，不是评分或固定坐标；实际位置随团队实现变化。横轴说的是代码进入生产主干的节奏，不是部署频率。纵轴只看主干之外的验证组织方式，主干开发仍会持续验证主干上的组合结果。GitHub Flow 也能在合并前部署测试环境。
