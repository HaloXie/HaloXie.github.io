...
假设你和同事正在用 AI 并行改进一个后台系统：一个 Agent 增加“导出报表”，另一个调整“查询接口”，第三个顺手实现刚想到的“筛选预设”。每个任务在自己的分支和工作目录里开发，进度却不按启动顺序排列：先启动的还在调试，后启动的已经提交了代码。

把导出和查询放到测试环境，导出却因接口返回格式变化而报错。你们排查时又发现一个历史 bug：回到当前生产版本仍能复现，即使不上新功能也应该尽快修复。

这时的任务状态可能是这样的：

| 改动 | 当前状态 | 发布安排 |
|---|---|---|
| 导出报表 | 实现已完成，组合测试失败 | 修复兼容性问题后重测 |
| 查询接口 | 仍在调试，新接口尚未放行 | 继续开发 |
| 筛选预设 | 后启动，先通过验收，且不依赖前两项 | 可以单独安排发布 |
| 历史 bug 修复 | 从生产主干单独拆出，优先验证 | 通过后优先上线 |

但 `develop` 已经混合了这些改动以及其他同事的功能。整体合回主干会把未放行代码一并带上；等所有任务结束，又会拖住已经可以上线的修复。

**AI 可以并行实现 idea，但“代码写完”“验证通过”和“允许发布”是三个不同状态。发布顺序应由验收结果和依赖决定，不由任务启动顺序决定。**

我们的做法是从 `master/main` 拆出独立改动，先在共享环境组合测试，再让已经放行的分支分别回主干，固定版本后上线。人或 AI 执行都遵循同一条路径；AI 自报完成不替代测试和审查。

这套做法在我们的团队实践中取得了良好效果。上面的 AI 并行任务是一个假设场景，用来说明它如何承接更多 idea 和不一致的完成顺序；它不是 Git 标准流程，也不要求团队必须使用 AI。
...
独立 worktree 是同一仓库的独立工作目录，用来隔离 Agent 的工作文件；它不会隔离共享测试环境。建议由集成人员或流水线串行管理 `develop` 的合并与部署，验证期间固定测试版本。

| 要记录的内容 | 防止什么误判 |
|---|---|
| 本次测试的 commit／产物摘要 | 测的是旧版，却把结果记到新提交上 |
| 验证范围、结果与确认人 | Agent 写完代码就被当作验收完成 |
| 环境占用与部署窗口 | 一个任务测试到一半，被另一个任务覆盖环境 |

版本变化后，旧结果只能证明旧版本；重新确认受影响范围并补测，再放行新的组合。
