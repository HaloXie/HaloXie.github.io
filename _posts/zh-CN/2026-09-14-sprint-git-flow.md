...
description: "基于一份团队内部流程文档整理的 Git 分支实践：它解决什么问题、如何运行、与常见模型有何不同，以及为什么它不是标准流程"
...
> 本文根据一份团队内部的《后端 Git Flow 规范》整理和脱敏。原文是团队协作规则，本文将它改写成面向工程读者的实践记录：保留流程背后的判断，删除具体组织、仓库、成员、版本和基础设施信息。

多人同时开发时，真正容易出问题的地方通常不是“有没有分支”，而是**几个改动第一次组合时，谁来验证它们能否一起工作**。每个人的分支单独看都通过了测试，合在一起却可能出现接口冲突、配置不兼容或数据库变更顺序错误。

这份内部实践给出的答案是：让一个短生命周期的 `develop` 承担 Sprint 集成测试，再让每个开发分支分别回到生产主干。

> **先说边界：这不是 Git 的标准流程，也不是行业统一规范。** 它只适合特定规模、发布节奏和测试条件的团队。采用前应结合仓库保护规则、CI 能力、测试环境和发布责任人验证；如果这些前提不成立，应选择其他模型。

## 它把什么问题放在了哪里

原文主干名为 `master`，本文统一写作 `main`，只改变名称，不改变职责。这里的 Flow 是普通的“协作流程”，并不指下文介绍的经典 GitFlow。
...
原文的规则可以归纳为：开发分支从主干创建；包括紧急修复在内，先进入集成测试；最终通过 MR 回主干；发布标签只在主干上创建。不要直接在共享分支上开发功能，也不要用强推回退共享主干。`develop` 的计划重建是一个单独协调的生命周期操作，不是允许随意回退历史。

### 先识别它最重要的盲点

假设 A 修改查询接口，B 增加一个供 A 调用的新接口。在 `develop` 上测试的是 `main + A + B`，但 A 的 MR 可能只准备发布 `main + A`。组合测试通过，无法证明 A 可以独立发布。

**集成环境的验证结果，不能自动替代目标发布组合的验证结果。** 这是对原文的补充分析，而不是原文已经具备的保障。实际采用时，应在 MR 上验证当前目标主干与本次改动的合并结果；存在依赖时，先合并并验证依赖，或者明确组成同一发布批次。做不到这一步，就不能把“develop 测过”当作独立上线的充分条件。
...
`feat/` 表示新功能，`fix/` 表示缺陷修复，`upd/` 表示非缺陷的增强或调整。这是本文案例的本地约定，Git 本身不要求这些前缀。
...
提交信息可参考 [Conventional Commits](https://www.conventionalcommits.org/en/v1.0.0/) 的 `type: description` 结构，常用类型包括 `feat`、`fix`、`refactor`、`docs`、`chore`。标题写清改动；涉及原因、兼容性或破坏性变化时，正文和 footer 仍有价值，不必把原文的“标题足够”理解为禁止提交正文。
...
在原文案例里，这次推送由 CI 自动部署到测试环境。这是仓库流水线的配置，不是创建一个名为 `develop` 的分支就自然具备的能力。

测试包括自身功能和与其他分支的交叉影响。原文允许在合入 `develop` 时解决集成冲突，但这个解决结果只存在于集成分支，不会自动进入开发分支。
...
原文采用“主干打标签 → CI 构建 → 人工部署”的发布方式。本文保留这一职责划分，省去内部标签前缀、通知渠道和部署平台。
...
原文把这套实践放在几个常见模型旁边比较。这个比较不是排名，而是说明取舍：
...
它把难点放在“如何持续保持主干可用”：快速自动化验证、可拆分的小改动，以及快速恢复机制。原文案例当时仍较依赖人工验证，自动化安全网不足，因此选择先在共享沙箱中组合测试。这是当时的取舍，不代表 Trunk-Based 不允许手工测试；随着反馈能力改善，也应重新评估是否还需要该沙箱。
...
|---|---|---|---|
| Trunk-Based | 小改动频繁集成主干，可用短命分支 | 快速反馈、可拆分改动、稳定 CI | 案例当时人工验证较多，自动化安全网不足 |
| GitHub Flow | PR 审查、检查后合入主干，可有预览环境 | 单条主线、轻量协作 | 本文另设跨 PR 的共享组合测试沙箱 |
| GitFlow | `feature → develop → release → master`，修复需回补 | 明确的发布准备阶段和生产维护需求 | 案例未采用长期 develop 与 release 层级 |
...
原文明确要求紧急修复同样先进入 `develop` 测试，再 MR 到主干，没有跳过测试的例外；变化的是优先级和响应速度。本文保留这一规则。若共享测试环境混有未发布功能，仍要检查修复在实际生产基线上是否成立。不能及时获得可靠验证环境时，说明这套 Flow 的前提不满足，需要团队另行制定应急流程，而不是把跳过步骤包装成本文原有规则。

### develop 重建：清理测试组合，不是丢掉开发工作

常规重建放在 Sprint 收尾；原文也允许在集成冲突难以处理或测试环境异常时协调重建，但重建本身不是定位根因的替代品。
...
流程主体来自内部《后端 Git Flow 规范》，已匿名化整理，不公开原文链接、截图与部署标识。模型辨析、发布组合的验证缺口，以及 Git 命令注意事项属于本文补充说明，并非原文规则或已实施效果。
