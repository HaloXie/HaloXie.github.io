...
这就是这套 Flow 最想保留的能力：**从主干拆出相互独立的改动，在共享环境里测试，再按实际准备程度分批回到主干上线。测试可以集中，发布不必捆绑。**

我在团队中实践和完善了这套做法，取得了良好的实践效果。它适合小团队按迭代集中测试，但不是 Git 标准流程。下面先解释环境与分支的关系，再看它与经典 GitFlow 的区别，最后走一遍具体操作。

## 先分清三个环境，以及它们运行的代码

环境是程序实际运行的地方，分支是 Git 中组织代码的一种方式。创建了一个分支，并不会自动多出一个运行环境；二者由团队的 CI/CD 流水线连接起来。
...
这套方式成立的前提，是修复确实能在生产基线上独立成立。如果它调用了同事尚未发布的新接口，就还没有拆干净：要么调整实现，使修复不依赖新功能，要么将依赖一起纳入明确的发布批次。
...
主干后来又有提交，不影响已经选定的 tag。环境配置分别注入，gray 和生产的依赖、数据差异仍要检查；不能因为代码相同就认为两个环境完全等价。建议记录 tag、commit、产物摘要和每个环境的部署记录，让“验证过的版本”和“上线的版本”可以对应起来。
