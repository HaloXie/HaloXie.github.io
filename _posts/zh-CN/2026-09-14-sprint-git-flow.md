...
**怎样让测试环境尽早集成，又确保生产只运行本次准备发布的版本？** 这是这套分支流程要解决的问题。
...
一次 **Sprint** 就是一轮约定好时间与目标的迭代。测试环境需要接纳本轮正在验证的改动，生产环境则只应接收已经放行的版本。这两个代码集合不一定相同。

| 分支或版本 | 绑定的环境 | 用来做什么 |
|---|---|---|
| `feat/*`、`fix/*`、`upd/*` | 默认不绑定独立环境 | 从主干创建，保存功能、修复或优化 |
| `develop` | 共享测试环境 | 推送触发 CI 部署，验证 Sprint 内多项改动的交叉影响 |
| `master/main` | 生产发布的代码来源 | 接收通过测试与审查、准备发布的改动；合并不等于立即部署 |
| 主干上的发布 tag | gray 和生产环境可选择同一版本 | 固定候选提交，构建、验证并发布可追踪的产物 |

`develop` 是代码分支，测试环境是运行代码的地方，二者通过 CI/CD 配置绑定。**环境可以有多个，长期分支不必同样多。** gray 和生产完全可以使用同一个主干 tag，区别在部署位置和放行阶段。

![开发分支先进入绑定的测试环境，验证后回到生产发布主干](/assets/img/sprint-git-flow/flow.webp)

这里有两条明确的合并关系：

1. 从 `master/main` 创建开发分支，避免继承测试分支上尚未准备发布的代码。
2. 开发完成后，先合入 `develop` 触发测试部署；验证通过，再由开发分支 MR 回主干。不要把共享测试分支整体当作生产发布包。

**MR（Merge Request）就是合并请求；GitHub 中叫 PR（Pull Request）。** Source 是提供改动的分支，Target 是接收改动的分支。这里选择功能分支作为 Source，主要是为了让生产接收已放行的改动，而不是把测试环境中的所有改动一并带入生产。

### 测试通过以后，还要验证本次准备发布的版本

回到报表例子：测试环境可能同时包含“导出报表”和“新查询接口”，但这次只准备发布导出。若导出依赖尚未发布的新查询接口，一起测试通过并不能证明它可以单独上线。依赖也必须纳入这次发布，或者调整功能使其兼容现有接口。

因此我们区分两个问题：`develop` 回答“这些改动放在一起能不能工作”，发布前的 gray 验证回答“本次准备上线的这一版是否可放行”。gray 的入口应是明确的候选版本，而不是自动接收整个 `develop`。
