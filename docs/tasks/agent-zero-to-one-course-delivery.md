...
课程目录和实时发布状态以 `_data/series.yml` 为准。建立本 Task 时已经完成第 01–04 课；后续 session 不依赖这里的静态进度，而是从 `_data/series.yml` 中第一个状态为 `planned` 的中文课程继续。

## 原书覆盖关系

28 篇短课不是按原书章节逐章翻译，而是重新按学习顺序组织。写作时仍必须从前言到第 16 章完整阅读，确保下面每一章都进入对应课程：

| 《Hello Agents》章节 | 对应课程 | 处理方式 |
| --- | --- | --- |
| 01 初识智能体 | 01–03 | 建立 Agent 定义、边界和架构地图 |
| 02 智能体发展史 | 03 | 用经典 Agent、LLM Agent、Multi-Agent 的演进解释架构来源 |
| 03 大语言模型基础 | 04–06 | 讲模型调用、Message、Token、Context 和最小 Loop |
| 04 智能体经典范式构建 | 15–19 | 用 Pi 重写主要 Agent Pattern |
| 05 基于低代码平台搭建 | 07 | 作为“可视化 Workflow 与代码 Agent”的边界案例，不复刻具体平台教程 |
| 06 框架开发实践 | 06–10、20 | 对照 Pi 解释 Loop、Tool 和框架接入方式 |
| 07 构建你的 Agent 框架 | 06、20–23 | 讲运行时、协议、安全、Trace 和 Evaluation 的最小框架能力 |
| 08 记忆与检索 | 13–14 | 覆盖 RAG 与四类 Memory |
| 09 上下文工程 | 11–14 | 覆盖 Context 组织、Session、Compaction、RAG 和 Memory |
| 10 智能体通信协议 | 19–20 | 同时说明 MCP 与 A2A；不把 MCP 误写成全部通信协议 |
| 11 Agentic-RL | 23 | 解释训练期优化与运行时 Agent 的边界、数据和评估闭环，不展开训练代码 |
| 12 智能体性能评估 | 23、27–28 | 建立 Trace、评估集、Evaluator 和发布门禁 |
| 13 智能旅行助手 | 24 | 作为完整 Agent 项目的需求拆分案例 |
| 14 自动化深度研究智能体 | 24–28 | 作为最终“证据优先技术研究 Agent”的直接项目主线 |
| 15 构建赛博小镇 | 19 | 作为 Multi-Agent、共享状态和角色协作案例 |
| 16 毕业设计 | 28 | 用可运行、可测试、可演示的最终交付收口 |

Extra-Chapter 和社区共创项目作为补充资料按需引用，不属于“完整读完原书 16 章”的硬门槛，也不因此继续扩充课程数量。
...
新 session 继续时：先读本文件和 `_data/series.yml` → 找到第一个状态为 `planned` 的中文课程 → 按“原书覆盖关系”阅读对应章节 → 补读上一课和 `AGENTS.md` → 用自己的话与 Pi 示例完成中文课程 → 发布后更新 `_data/series.yml`。如果刚完成一个大阶段，先做“小白 Agent”连续阅读验收，再进入下一阶段。
