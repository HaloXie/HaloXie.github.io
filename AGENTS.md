...
- 新文章或文档转博客前，先读 `README.md`，再完整阅读至少两篇与目标读者、文章类型接近的既有文章，并实际查看其配图。优先参考 `_posts/zh-CN/2026-08-13-chat-workflow-agent-guide.md` 的案例引入与概念解释、`_posts/zh-CN/2026-07-28-chezmoi-secure-dotfiles-guide.md` 的操作衔接；流程实践可参考 `_posts/zh-CN/2026-09-14-sprint-git-flow.md`。参考叙述方式和视觉结构，不照搬未经核实的事实，也不把某篇文章当成不可修改的模板。
- 起草前用一句话锁定核心命题：读者遇到什么矛盾，这种做法凭什么解决，最关键的取舍是什么。案例必须暴露这个矛盾，不能用一个次要收益替代核心价值。例如分支流程的重点是共享测试与差异化上线，不能缩减成“审查者看到更小的 diff”。
- 从材料改写时，先在工作记录中列出核心规则、成立前提、状态/对象关系、正常路径、异常恢复和适用边界，对照成稿逐项核验。可合并重复命令，但不能删掉与核心命题、操作闭环或安全边界直接相关的发布、恢复或生命周期内容；与主题无关的材料章节可以省略或移入参考。不把工作记录和改写过程塞进正文。
- 原材料含画板、图片或内嵌图表时，必须实际打开逐张阅读；只取得 token、Markdown 占位或图片 alt 不算读过。先判断原图的结构、分支与回路是否更清楚，再决定沿用、脱敏重绘或替换。保留有效表达，不为了统一颜色而重新发明流程；无法读取的图明确列为未核实，不宣称来源已完整覆盖。

### 按人的理解顺序展开

- 默认主线：短案例与冲突 → 最少必要概念/对象关系 → 一张总图与步骤速查 → 关键边界；完整命令、方案变体和恢复手册按需查阅。主线应让读者不展开附录也能回答“解决什么、对象如何关联、从哪开始、何时放行、失败去哪”，不要求通读全文才理解做法。知识解释型文章可按主题调整，不机械套流程教程。
- 一个案例贯穿动机、动作与结果；确有两种不同场景时，用 `Case 1 / Case 2` 的明确标题分别写“遇到什么 → 怎么处理 → 结果怎样”，不要把多人协作、AI 并行、历史 bug 和发布变体全部塞进开头。假设案例就近标明，不伪装成真实事故；MR、CI、Sprint 等关键术语首次出现就近解释。
- 每段只推进一个判断，不让案例反复被定义、模型百科和命令打断。图表表达过的路径不再逐节点复述成长段；总图配速查表可以互补，两个同维度表或多个同义总结应合并。控制的是读者必须连续处理的信息量，不以字数、图数或“阅读分钟数”代替可读性判断。
- 操作细节、异常速查和备选方案可以用清楚命名的 `<details markdown="1">` 折叠，提供定位入口；不可把决定是否适用、依赖是否满足、能否发布的关键条件藏起来。折叠不是精简的替代品：先删重复，再分层，并在最终页面验证默认状态、展开后的 Markdown/代码渲染及目录跳转。
- 操作段必须承接前文状态：当前起点与运行状态（如所在分支/环境）、对象已有还是新建、由谁执行、产物去哪、通过什么检查才进入下一步。前文提出的重要条件必须落在对应操作中；不能前文要求验证候选版本，后文却只写“测试通过就合并”。避免让同一案例重复创建已经存在的对象。
- 修正核心理解时重整受影响的叙述、例子、表格和图，不靠追加免责声明或括号补丁维持旧框架。删掉重复总结、同维度重复表格，以及不帮助读者行动的编辑说明。
- 比较多个方案时优先用同维度表格，避免逐项长篇介绍后再重复整张表。只有两个清晰维度确实能解释适用场景时才加定位图；定性点位须说明典型场景与边界，不伪造量化评分或普遍“最佳位置”。采用中心象限图时，轴交点置于画布中心；“0”只能表示明确的中性分界，不能暗示能力零分。图负责概览，表负责查阅，正文只补关键因果与例外。

### 忠于作者实践，准确解释差异

- 明昊自己的材料改写为博客时，以作者的第一手实践口吻展开。除非明确要求交代文档沿革，不写“根据内部文档脱敏”“这是对原文的补充”“并非原文已经具备的保障”等幕后说明。脱敏落实在移除身份、内部链接和业务标识；截图若能安全匿名化就重绘或遮盖，无法安全处理时再删除，不把脱敏过程变成文章主题。公开外部资料仍应正确引用，不能抹去他人来源。
- 区分已确认实践、可选建议和通用事实。不得擅自把建议升级为现行流程或必备基础设施，也不得替原流程新增绕过规则的例外。效果表述只使用作者确认或证据支持的结论，不补造指标。
- 涉及 Git 协作与发布流程时，先区分对象与运行状态：分支不等于环境，合并不等于部署，tag 不等于已经上线，组合测试不等于所有子集都验证通过。图与文字应分别讲清代码路径、部署绑定、验证条件、发布单位及回补关系；不要求每个环境必有一个长期分支。
- 增加 AI 场景时，用它说明并行度、任务完成顺序与验收顺序的变化，不把通用工程主题改写成 AI 宣传。实现完成、验证通过、允许发布分开表达；AI 自报完成不替代验收，独立工作目录不等于外部环境隔离，测试结论绑定具体版本。
- 对比主流做法时，先查各自的一手资料，若主题是 Git 或发布流程，按相同维度比较起点、流转、验证、交付单位、生命周期和成本；其他主题选择与其决策直接相关的共同维度；优先突出本方案的实质差异。不得为抬高本方案制造稻草人，例如“GitHub Flow 不能合并前测试”“经典 GitFlow 不能单独 hotfix”。实践方案须交代适用条件，不能包装成行业标准。

### 独立试读与内容交付

- 文章须适配已有重点高亮能力，写前阅读 `plugins/reading-highlights/README.md`。把少量承重结论写成自然、独立、以加粗为主的段落；不要整篇加粗或为凑命中拉长句子。插件按节限额且排除代码、列表等结构，不能把“写了粗体”等同于“重点已高亮”。浏览器检查开关状态及实际 `.reading-highlight` 命中文案，确认核心命题被选中、案例细节没有抢占主结论；同一语义在英译中保持，不改插件阈值来迁就单篇文章。
- 新增长篇教程、流程实践或整体重写后，安排 2–3 个独立 agent 模拟不同起点的读者（如只懂基础命令、初次团队协作、一般技术读者）。让他们只凭成稿复述核心问题、对象关系、正常路径、关键差异和失败处理，指出具体卡点；不要只问“写得好不好”。改完共同卡点再复读，结果明确标为 agent 模拟试读，不冒充真人用户验证。简单勘误不需重复整套试读。
- 逐项对照来源完整性与技术正确性，再核对试读是否能解释核心命题；“没有找到错误”不等于读者已经理解。不能用机械门禁或 agent 好评替代作者对核心语义的判断。
- 文本密度、段落或理解顺序被指出有问题时，独立编辑审读应给出具体删减与重排建议，并与站内范例比较；不能只增加标题、把所有段落改成表格或继续叠加解释。复查短主线是否独立成立，必要的回路和例外是否仍可找到。
- 新文章交付须运行 `scripts/check-translations.rb`、`scripts/check-series.rb`、`scripts/check-reference-links.rb`、`scripts/check-content-images.rb` 和 `git diff --check`；发布验收还须有生产构建、搜索索引生成后的 `scripts/check-rendered-site.rb` 及站点链接检查证据。新增图片目录按 `_config.yml` 的共享资源约定处理；新增文章跨过分页阈值时核验分页链接、canonical 与 sitemap，不通过删除门禁放行。
- 本地依赖阻塞要写明，允许已有 CI 承担完整构建验证；推送不等于发布成功。获准发布后，跟踪对应提交的正式部署流水线，再检查线上文章、关键图片与修改后的文案。不能把旧缓存页面、生成失败后残留的旧图片或仅通过尺寸检查的文件当作新结果。
...
- 修改文章后必须扫描 `_posts/` 中的 `npm install` 和 `npm i`；当前新增或修改文章中的无例外命中必须在交付前改为等价 pnpm 命令，历史文章命中单独记录，不因本次改稿顺手扩散范围。
...
- Banner 与正文图必须分工：Banner 用简洁概念与视觉隐喻吸引阅读，正文图解释具体关系和步骤。禁止直接复用同一张图造成开篇连续重复，也不能裁切无关旧图充当新封面。
- 正文图只放在认知负担高的节点，优先级为：
  1. 架构或模块关系；
  2. 流程、状态机或时间顺序；
  3. 多维对比或决策框架；
...
- 总览图可以放在首次解释之后帮助读者建立地图；解释具体节点的细图应紧跟对应概念，不能提前剧透尚未解释的结论。
- 图表先看站内成品再设计；`assets/img/agent-patterns-with-pi/pi-minimal-loop.webp` 与 `assets/img/chezmoi-secure-dotfiles/recovery.webp` 可作为精确信息图参考。保持本站配色、线条、字号层级与留白，不以“换成米白色”代替布局设计。
- 流程图只保留关键节点，用阶段、泳道或卡片减少交叉与长链；箭头必须有明确方向和语义，实线/虚线表示不同关系时给图例。复杂细节放正文，图不能靠小字复刻整段规则。Mermaid 适合简短关系；若使用，必须启用 `mermaid: true` 并验证实际渲染，不能将默认黑底长图直接当作本站视觉成品。
- 流程里有判断就画出通过与不通过的去向；失败返回修改、发布后检查与恢复不能被一串成功步骤替代。代码合并、部署、测试放行是不同关系，不能用一根未标注的箭头混写。图中编号与步骤表保持一致，避免箭头穿过卡片、相交歧义或标签遮挡。
- 各语言图使用同一结构与视觉角色，分别检查文字长度、换行及裁切；修改流程语义时，同时检查已有各语言图文是否漂移。语言发布与配对遵循上文“中文先行、定稿后英译”。
...
- 导出成功后逐张打开最终 WebP 检查，并核对对应 source 与引用；字体/导出命令失败必须修复，不能忽略退出码后继续复制旧产物、提交或宣布完成；
- 在最终文章中检查桌面和 360px 阅读效果、Light/Dark、图片放大入口；无横向溢出不代表字可读，缩小后看不清主关系就应减少节点或拆图；
